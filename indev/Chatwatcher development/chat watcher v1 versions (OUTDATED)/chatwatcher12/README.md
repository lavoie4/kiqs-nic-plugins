...
- **Message Exceptions** — a whitelist of ignored users whose messages still show **in public rooms only** (covers both name- and IP-ignored users). Their private messages are **hard-blocked at the code level** (`incoming_private_chat_event` returns `zap`), so they genuinely can't DM you — not just a documented exception.
...
Both tabs attach to Nicotine+'s main notebook (GTK 3 and GTK 4 compatible). To keep Nicotine+ from crashing with `KeyError('<tab id>')` when you switch to a plugin tab, the plugin patches `MainWindow.set_active_header_bar` to skip its own tab ids (the single funnel every header-bar/toolbar swap goes through), and the "Ignore for…" menu + message-exceptions whitelist monkey-patch `UserPopupMenu.setup_user_menu` and `NetworkFilter.is_user_ignored` / `is_user_ip_ignored`. Whitelisted users' private messages are dropped in `incoming_private_chat_event` (returns `returncode["zap"]`). All patches are restored when the plugin is disabled.
