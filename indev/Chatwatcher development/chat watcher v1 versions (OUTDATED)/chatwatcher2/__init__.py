...
"""Chat watcher v1 — Listenings + Keyword Watch + Shitlist, merged.
...
1. **Listenings**  — collect other users' ``/nowplaying`` (``/me``) output into
   a "Listenings" tab, with retention and user blocking.
2. **Keyword Watch** — collect other users' messages that mention your keywords
   into a "Keyword Watch" tab, in public rooms and private chat.
...
class Plugin(BasePlugin):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.metasettings = {
            ...
            "kw_keywords": {
                "description": "Keywords to watch for (one per entry, e.g. your username):",
                "group": "Keyword Watch",
                "type": "list string",
            },
            "kw_case_sensitive": {
                "description": "Match keywords case-sensitively",
                "group": "Keyword Watch",
                "type": "bool",
            },
            "kw_clear_log": {
                "description": "Clear the Keyword Watch log now",
                "group": "Keyword Watch",
                "type": "bool",
            },
            "kw_blocked_users": {
            ...
            "shitlist_show_messages": {
                "description": (
                    "Ignored users whose messages are still shown. Type the name of a user "
                    "you've ignored but still want to hear from. One name per line."
                ),
                "group": "Message Exceptions",
                "type": "list string",
            },
            "shitlist_users": {
                "description": (
                    "Users currently banned and ignored. Add a name to ban and ignore it, "
                    "remove a name to unban and unignore it. Changes apply when you click Apply."
                ),
                "group": "Shitlisted Users",
                "type": "list string",
            },
        }
        ...
        self.commands = {
            "kw": {
                ...
                "description": "Manage Keyword Watch keywords (add/remove/list)",
    ...
    def init(self):
        self._listenings_migrate_legacy_log()
    ...
    def loaded_notification(self):
        ...
        if keywords:
            ...
            self.log(
                "Keyword Watch has no keywords yet. Add some (e.g. your username) in "
    ...
    # ------------------------------------------------------------------ #
    # Chat hooks — Listenings (pre-strip /me events)
    # ------------------------------------------------------------------ #

    def incoming_public_chat_event(self, room, user, line):
        if line.startswith("/me "):
            self._listenings_add_entry(user, line[4:].strip(), context=room)
    ...
    # ------------------------------------------------------------------ #
    # Chat hooks — Keyword Watch + Shitlist (post-processing observers)
    #
    # Both hooks fire *after* Nicotine+'s ignore filter. A message from an
    # ignored user is normally dropped (msg.room/msg.user set to None) before
    # any plugin hook runs — but Shitlist's "show messages from ignored users"
    # whitelist patches NetworkFilter.is_user_ignored to return False for those
    # names, so their messages flow through and reach Keyword Watch too. That's
    # what makes ignored-but-whitelisted users' keyword hits still appear in the
    # Keyword Watch tab.
    # ------------------------------------------------------------------ #
    ...
    def _listenings_log_path(self):
        return os.path.join(self._logs_dir(), LISTENINGS_LOG_FILENAME)

    def _listenings_migrate_legacy_log(self):
        """Move the old log (previously stored directly in the plugin folder)
        into the new logs/ subfolder."""

        if not self.path:
            return

        old_path = os.path.join(self.path, LISTENINGS_LOG_FILENAME)
        new_path = self._listenings_log_path()
        ...
        if os.path.isfile(old_path) and not os.path.exists(new_path):
            try:
                ...
                self.log("Could not migrate log file: %s", error)
    ...
    def _listenings_save(self):
        try:
            ...
            self.log("Could not write Listenings log: %s", error)
    ...
    def _kw_log_path(self):
        return os.path.join(self._logs_dir(), KEYWORDWATCH_LOG_FILENAME)
    ...
    def _kw_save(self):
        try:
            ...
            self.log("Could not write Keyword Watch log: %s", error)
    ...
    def _kw_clear(self):
        ...
        self._kw_save()
        self._kw_render()
        self.log("Keyword Watch log cleared.")
    ...
    def _shitlist_sync_users_list(self):
        ...
        self.settings["shitlist_users"] = sorted(banned & ignored)
    ...
    def _shitlist_patch_ignore_filter(self):
        ...
        original = network_filter_class.is_user_ignored  # unbound function
        plugin = self
        ...
        def patched_is_user_ignored(nf_self, username):
            ...
            return original(nf_self, username)

        network_filter_class.is_user_ignored = patched_is_user_ignored

        self._shitlist_original_is_user_ignored = original
    ...
    def _shitlist_ban_and_ignore(self, user):
        ...
        ignored = self._shitlist_is_actually_ignored(user)
        ...
        return (not banned) or (not ignored)
    ...
    def _shitlist_ban(self, user, keyword):
        ...
        if self._shitlist_ban_and_ignore(user):
    ...
    def _kw_list_keywords(self):
        ...
        if not keywords:
            self.output("No keywords on the Keyword Watch list.")
            return True

        self.output("Keyword Watch keywords (%s):" % len(keywords))
    ...
    def _open_kw_settings(self):
        ...
        if application is None:
            self._kw_list_keywords()
            self.output("(Open Preferences -> Plugins -> Chat watcher v1 -> Settings to edit keywords.)")
            return False
        ...
        try:
            ...
            dialog = getattr(application, "_chatwatcher_kw_dialog", None)

            if dialog is None:
                dialog = PluginSettings(application)
                application._chatwatcher_kw_dialog = dialog
    ...
    def _shitlist_command(self, args, user=None, room=None):
        ...
        if not args:
            return self._open_shitlist_settings()
    ...
    def _open_shitlist_settings(self):
        application = self._get_application()

        if application is None:
            self._shitlist_list_keywords()
            self.output("(Open Preferences -> Plugins -> Chat watcher v1 -> Settings for the keyword editor.)")
            return False
        ...
        try:
            ...
            dialog = getattr(application, "_chatwatcher_shitlist_dialog", None)

            if dialog is None:
                plugin = self

                class ChatWatcherShitlistSettingsDialog(PluginSettings):
                    ...
                    def on_ok(self_, *_args):
                        ...
                        for username in sorted(new_users - old_users):
                            if plugin._shitlist_ban_and_ignore(username):
                ...
                dialog = ChatWatcherShitlistSettingsDialog(application)
                application._chatwatcher_shitlist_dialog = dialog
    ...
    def _build_listenings_page(self, feed):
        ...
        clear_button = self._make_toolbar_button(
            ...
            "Delete all entries from the Listenings log"
        )
        ...
        open_folder_button = self._make_toolbar_button(
            ...
            "Open the folder where the Listenings log is stored"
        )
        open_folder_button.connect("clicked", self._on_open_logs_clicked)
    ...
    def _build_keywordwatch_page(self, feed):
        ...
        clear_button.set_tooltip_text("Delete all entries from the Keyword Watch log")
        ...
        open_folder_button.set_tooltip_text("Open the folder where Chat watcher logs are stored")
        open_folder_button.connect("clicked", self._on_open_logs_clicked)
    ...
    def _on_open_logs_clicked(self, *_args):
