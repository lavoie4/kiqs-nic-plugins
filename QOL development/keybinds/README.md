...
| Toggle current tab (hide/show) | `Win+Shift+H` |
...
- `/tabs` — list, hide, show, or toggle main tabs:
  - `/tabs` or `/tabs list` — list every main tab with its visible/hidden state.
  - `/tabs hide <tab>` / `/tabs show <tab>` / `/tabs toggle <tab>` — hide, show,
    or toggle a tab. `<tab>` can be the tab title (e.g. `Played`, `Search Files`)
    or its id (e.g. `listenings`, `search`).
  - `/tabs reset` — show all tabs again.

Hidden tabs are only hidden, never removed — they keep collecting data (for
example the `Played` tab keeps logging, and Search keeps running). Hidden state
persists across restarts.

## Export log

Right-click the log pane to open its context menu, then choose **Export Log**.
This opens a native folder picker; pick a folder and the current session log
(everything shown in the log pane) is saved as `nicotine-log-<timestamp>.txt`
in that folder.
