...
class Plugin(BasePlugin):
    ...
    def _listenings_render(self):
        ...
        self._apply_find_highlight(feed)
    ...
    def _kw_render(self):
        ...
        if label is not None:
            ...
            if filter_mode != "All":
                label.set_text(f"{shown} {suffix} ({filter_mode})")
            else:
                label.set_text(f"{shown} {suffix}")
    ...
    def _shitlist_render(self):
        ...
        if label is not None:
            ...
            if imported is not None:
                label.set_text(f"(imported) {count} {suffix}")
            else:
                label.set_text(f"{count} {suffix}")
    ...
    @staticmethod
    def _highlight_markup(text, query):
        from gi.repository import GLib
        ...
        pattern = re.compile(re.escape(esc_query), re.IGNORECASE)

        return pattern.sub(
            lambda m: '<span background="#FFD500" foreground="#000000">%s</span>' % m.group(0),
            esc,
        )

    def _iter_entry_buttons(self, list_widget):
        from gi.repository import Gtk
        ...
        if self._is_gtk4():
            ...
            while child is not None:
                button = child.get_child() if isinstance(child, Gtk.ListBoxRow) else child
            ...
            for child in list_widget.get_children():
                button = child.get_child() if isinstance(child, Gtk.ListBoxRow) else child

                if button is not None:
                    yield button

    def _apply_find_highlight(self, feed):
        from gi.repository import Gtk

        list_widget = feed.get("list_widget")

        if list_widget is None:
            return

        query = (feed.get("search_query") or "").strip()

        for button in self._iter_entry_buttons(list_widget):
            ...
            markup = self._highlight_markup(text, query)
