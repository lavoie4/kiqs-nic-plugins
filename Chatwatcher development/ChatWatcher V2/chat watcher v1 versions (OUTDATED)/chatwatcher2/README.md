...
One plugin that merges **Listenings**, **Keyword Watch**, and **Shitlist** into a single install, keeping every feature of the originals. The three feature sets are fully independent: each has its own tab/log/settings/command, and the settings are namespaced so nothing collides.

## What it does

### 1. Listenings

Collects other users' `/nowplaying` output (which Nicotine+ renders as `* <user> <text>`) into a **"Listenings"** tab, saved to a log file, with user blocking and retention control.

```
10:31:59 AM * Pa is having an eargasm to: Sunn O))) - Fried Eagle Mind (9:46)
```

- Detects `/me` actions via the pre-strip `incoming_public_chat_event` / `incoming_private_chat_event` hooks (and your own via the `outgoing_*` events, marked "(you)").
- Log file: `logs/listenings.log` (JSON-lines).
- Retention: `20 min` … `7 days` (default `1 day`).

### 2. Keyword Watch

Collects messages from *other* users that mention your keywords (e.g. your username) into a **"Keyword Watch"** tab, in public rooms and private chat.

```
10:31:59 AM [main] <alice> hey ambrose, check this out  [kw: ambrose]
```

- Hooks `incoming_public_chat_notification` / `incoming_private_chat_notification` (pure observers).
...
| Group | Options |
|---|---|
| **Listenings** | Clear log (checkbox), Users to hide (`list string`), Keep entries for (`dropdown`) |
| **Keyword Watch** | Keywords (`list string`), Match case-sensitively (`bool`), Clear log (checkbox), Users to hide (`list string`), Keep entries for (`dropdown`) |
| **Shitlist** | Enable banning (`bool`), Keywords (`list string`) |
| **Message Exceptions** | Ignored users whose messages still show (`list string`) |
| **Shitlisted Users** | Users currently banned+ignored (`list string`) |
...
1. **Hooks** — Listenings uses the four pre-strip `*_chat_event` hooks to catch `/me`; Keyword Watch and Shitlist share the two `incoming_*_chat_notification` observers.
...
|---|---|
| Tab names (`Listenings`, `Keyword Watch`) | `LISTENINGS_TAB_NAME` / `KEYWORDWATCH_TAB_NAME` near the top of `__init__.py` |
