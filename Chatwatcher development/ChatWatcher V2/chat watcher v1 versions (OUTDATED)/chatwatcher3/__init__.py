...
class Plugin(BasePlugin):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.commands = {
            "chatwatcher": {
                ...
                "description": "Chat watcher v1 overview, status, and settings",
                "parameters": ["[help|status|settings]"],
            },
            "cw": {
                "callback": self._chatwatcher_command,
    ...
    #
    # The global feed bypasses Nicotine+'s ignore filter, so we apply it here to
    # keep genuinely-ignored users hidden. Shitlist's "show messages from
    # ignored users" whitelist patches NetworkFilter.is_user_ignored to return
    # False for those names, so their messages still flow through.
    # ------------------------------------------------------------------ #
    ...
    def _open_played_settings(self):
        ...
        if application is None:
            self._played_list()
            self.output("(Open Preferences -> Plugins -> Chat watcher v1 -> Settings to edit Played options.)")
            return False
        ...
        try:
            ...
            dialog = getattr(application, "_chatwatcher_played_dialog", None)

            if dialog is None:
                dialog = PluginSettings(application)
                application._chatwatcher_played_dialog = dialog
    ...
    def _chatwatcher_command(self, args, user=None, room=None):
        ...
        if action == "settings":
            # Open the full settings dialog (includes the Shitlisted Users
            # reconciliation that plain /kw settings lacks).
            return self._open_shitlist_settings()

        self.output("Usage: /chatwatcher [/cw] [help | status | settings]")
        return False
    ...
    def _chatwatcher_help(self):
        ...
        self.output("  Shitlist      bans+ignores keyword users; adds 'Ignore for...' and message exceptions")
        self.output("")
        self.output("Commands:")
        self.output("  /cw [help|status|settings]         this plugin")
    ...
    def _chatwatcher_status(self):
        ...
        self.output("")
        self.output("Use /cw settings for the settings dialog; /played, /kw and /shitlist for the three features.")
        return True
