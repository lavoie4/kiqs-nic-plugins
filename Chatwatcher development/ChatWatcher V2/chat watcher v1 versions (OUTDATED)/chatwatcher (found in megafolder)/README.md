...
- **Keyword banning** — a message containing a Shitlist keyword bans+ignores the sender **by IP** (a hard block, so re-creating their account won't evade it). Every ban is logged with the username and IP.
...
- **Open Logs Folder** — a button in the settings dialog (`/cw`) that opens the folder where the logs live (including `shitlist.log`).
