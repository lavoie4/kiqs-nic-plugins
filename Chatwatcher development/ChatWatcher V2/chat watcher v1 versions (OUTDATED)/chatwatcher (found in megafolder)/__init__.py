...
class Plugin(BasePlugin):
    ...
    def _shitlist_ban_and_ignore(self, user):
        """Ban and ignore a user by IP (hard block). Returns (changed, ip)."""
        ...
        banned = self.core.network_filter.is_user_ip_banned(user)
        ignored = self.core.network_filter.is_user_ip_ignored(user)
    ...
    def _shitlist_ban(self, user, keyword):
        ...
        if changed:
            self.log("Banned and ignored user '%s' by IP (%s) for saying keyword '%s'", (user, ip, keyword))
    ...
    def _shitlist_log_path(self):
        return os.path.join(self._logs_dir(), SHITLIST_LOG_FILENAME)
    ...
    def _shitlist_unban(self, user):
        ...
        if changed:
            self.log("Unbanned and unignored user '%s' by IP", (user,))
            ...
            if dialog is None:
                ...
                class ChatWatcherSettingsDialog(PluginSettings):
                    ...
                    def update_settings(self_, plugin_id, plugin_settings):
                        ...
                        self_._add_open_logs_button()

                    def _add_open_logs_button(self_):
                        button = plugin._make_toolbar_button(
                            "folder-open-symbolic", "Open Logs Folder",
                            "Open the folder where Chat watcher logs are stored"
                        )
                        button.connect("clicked", plugin._on_open_logs_clicked)
